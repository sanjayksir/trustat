...
//========= js Demo Chunk upload code ===========

function demofilesValidation(type){
	$('#extra-progress-wrapperID').hide();
	var f2 				=	$.trim($("#demovideoFile").val());
	var validateFlag 	= 	true;
	if (f2 == null || f2 == "") {
		$('#extra-progress-wrapperID').show();
		$('#demovideoFile_err').html("Please choose video file");
	  	$('#demovideoFile_err').show();
		validateFlag = false;
	}else{
		var ext = f2.substring(f2.lastIndexOf('.') + 1);
		ext 	=	ext.toLowerCase();
		if(ext != type ){//&& ext != "jpg" && ext != "png" && ext != "gif" && ext != "mp3"){
 			$('#innerDivHide').hide();
			$.trim($("#demooffStart").val('0'));
			$.trim($("#demovideosupload").val(''));
			$.trim($("#ugc_video_size").html(''));
			$('#demovideo_progBar').css('width','0%'); 
			$('#videoProgBar').hide();
			$('#extra-progress-wrapperID').show();
			$('#demovideoFile_err').addClass('ugc_browseerror');
			$('#demovideoFile_err').html("Please choose only "+type+" file");
			$('#demovideoFile_err').show();
			validateFlag = false;	 
		}else{
			var files = document.getElementById('demovideoFile').files;
			var demovideofile = files[0];
			var fileDivideSize	=	1*1024*1024;
			var sizevideo =( demovideofile.size/fileDivideSize).toFixed(2);
			if(sizevideo<=200){	
				var demovideofilename = '';//videofile.name;
				$('#ugc_video_size').show();
				//document.getElementById("ugc_video_size").innerHTML ="Size "+(videofile.size/fileDivideSize).toFixed(2)+" MB video uploading";									
				document.getElementById("demofilename").innerHTML = demovideofilename.substring(0, demovideofilename.lastIndexOf(".") + 0);	
				$('#innerDivHide').show();	
				$('#extra-progress-wrapperID').hide();			
				$('#demovideoFile_error').hide();
				$('#demovideoFile_error').html();
				$('#demovideoFile_err').hide();
				if(validateFlag) validateFlag 	= 	true;
			}else{
				$('#extra-progress-wrapperID').show();
				html("Please choose max 200 MB file");
...
function ajaxuploadDemoFile(type){
	$('#demovideo_progBar').css('width','0%');
	document.getElementById("demovideosupload").value="";
	document.getElementById("demooffStart").value=0;
	document.getElementById("demovideosupload").value="";
	document.getElementById("ugc_video_size").value="";
	$("#demovideoupload").hide();
	var counter = 1;
	var fileFlag = demofilesValidation(type);
	if(fileFlag){
    closeStatus = 0;
	readBlob('demovideoFile');
	$("#hideBrowseButton").hide();
	$("#removeFade").hide();
	}
}
...
function readBlob(id){
	...
	var fileDivideSize	=	1*1024*1024;
	if(document.getElementById("demooffStart").value >1){
	  counter = document.getElementById("demooffStart").value;
	  start = counter*end;
	  end = start + 2*1024*1024;
	  counter++;
	}else{
    ...
    demovideofile = files[0];
	dots = demovideofile.name.split(".")
	fileType = "." + dots[dots.length-1];
	//document.getElementById("filesize").innerHTML = (videofile.size/fileDivideSize).toFixed(2)+" MB";
	totalChunk = Math.ceil(demovideofile.size/end);
	vfilename = "video_"+vid+"_"+videotime+fileType;
	document.getElementById("demovideonewfile").value=vfilename;
	document.getElementById("demosavetoken").value="video_"+vid+"_"+videotime;
	
	resumableIdentifier = 'video_'+videotime;
	//demomovePointer(counter,start,end);
	demomovePointer(counter,start,end,totalChunk);
}

function demomovePointer(counter,start,end,totalChunk){
	var blob = demovideofile.slice(start, end);
	if( start < demovideofile.size ) {
		demouploadFile(blob, counter,demovideofile.size,vfilename,totalChunk,end);
		//$("#videosaction").prop('disabled', true);
	}else{	
		//success
		if(closeStatus==0){
		document.getElementById("demooffStart").value = 1; 
		document.getElementById("demovideoFile").value="";
		document.getElementById("demovideosupload").value="sucess";
		$('#innerDivHide').show();
		$('#demovideoupload').show();
		//$("#videosaction").prop('disabled', false);
		}
	}	
}

function demouploadFile(blobFile, part,fsize,name,totalChunk,end) {
	$("#demoprogressHide").hide();		
	var videoBaseUrl 	= 	$.trim($("#videoBaseUrl").val());	
	var fd = new FormData();    
	fd.append( 'file', blobFile );
	fd.append( 'resumableIdentifier', resumableIdentifier );
	...
	var chunkdate = new Date();
	var chunktime = chunkdate.getTime();
	 $('#videoProgBar, #demoprogressHide').show();	
	$.ajax({
		url:"<?php echo base_url();?>backend/upload_file/upload/"+chunktime,
		 data: fd,
		 processData: false,
		 ...
		 success: function(data){
			   if(data==1){
					...
					$('#extra-progress-wrapperID').show();
					$('#demovideoFile_err').addClass('ugc_browseerror');
					$('#demovideoFile_err').html("Please choose only mp4 video file");
					$('#demovideoFile_err').show();
				}else{
					if(closeStatus==0){
						document.getElementById("demooffStart").value = part;
								
						part++;
						start = end;
						end = start + 2*1024*1024;
						demomovePointer(part,start,end,totalChunk);
						 demoprogress(progressVal, $('#videoProgBar'));	 
						 $('#demovideo_progBar').css('width',progressVal+'%');
						 $("#demovideo_progBar").text(progressVal+'%');
						
						
						if(parseInt(progressVal)==100){
 							<!----------------Comma Separated Value--------------------->
							var demovideonewfile = $("#demovideonewfile").val(); 
							allids=[];
							allids.push(demovideonewfile);
							$('#all_product_demo_video_list').val(allids.join(","));
							<!----------------Comma Separated Value--------------------->
  							//$("#progressHide").hide();						
							//$("#demovideoupload").show().text('Video successfully uploaded.').css('color','green');
							//$(':input[type="submit"]').prop('disabled', false); 
 						}
					}
				}
		 ...
		 error: function (demoajaxContext) {
			//alert(ajaxContext.responseText)
			console.log("netIssue");
		}
	});
  }
  function demoprogress(percent, $element) {
   $('#progBar').attr('value',percent);
  }
  window.addEventListener('online',  demoresumeUpload);
 function demoresumeUpload(){
	var f2	=	$.trim($("#demovideoFile").val());
	if(f2 != null || f2 != "") {
	  end = 2*1024*1024;
	  counter = document.getElementById("demooffStart").value;
	  ...
	  var demovideofile = files[0];
	   var divideSize	=	2*1024*1024;
		totalChunk = Math.ceil(demovideofile.size/divideSize);
	  demomovePointer(counter,start,end,totalChunk);
	}
}
function demovalidateUgcVideo()
{
	var validateFlag = true;
   	if ($.trim($("#demovideosupload").val()) == '') {
		$('#extra-progress-wrapperID').show();
	  	$('#demovideoFile_err').html("Please choose video file");
		$('#demovideoFile_err').show(); 
		$('#demovideoFile_err').fadeOut(4000); 
        validateFlag= false;
   }   
	else if ($.trim($("#demovideoHeadline").val()) == '') {	
		$("#demovideoHeadline").focus();
		$('#demovideoHeadline_err').show(); 
		$('#videoHeadline_err').fadeOut(3500); 
        validateFlag= false;
   }    
	else if ($.trim($("#videosCategory").val()) == '') {
       ...
       validateFlag= false;		
			...
			var videoHeadline			= 	encodeURIComponent($.trim($("#videoHeadline").val()));
			var videosKeywords			= 	encodeURIComponent($.trim($("#videosKeywords").val()));
			var demosavetoken				= 	$.trim($("#demosavetoken").val());
			var folder_path				= 	$.trim($("#folder_path").val());
			var demovideonewfile			= 	$.trim($("#demovideonewfile").val());			
			var videoDescription		= 	encodeURIComponent($.trim($("#videoDescription").val()));
			var videoBaseUrl 			= 	$.trim($("#videoBaseUrl").val());	
			var fname 					= 	$.trim($("#fname").val());
			var state_name 				= 	$.trim($("#state_name").val());
			...
			//$("#videosaction").prop('disabled', true);		
	           $.ajax({
                type:"POST",
                        url:videoBaseUrl+"ugc-save/",
                        data: {videosCategory:videosCategory, videosCategoryOption:videosCategoryOption, videoHeadline:videoHeadline, demosavetoken:demosavetoken, demovideonewfile:demovideonewfile,videoDescription:videoDescription,videosKeywords:videosKeywords,folder_path:folder_path,requestType:'savevideo',uid:uid,fname:fname,state_name:state_name,city_name:city_name,country:country},
                        success: function(response) {						
							$("#video_loader").css("display", "none");
							if ( response.indexOf("saved") > -1 ) {							
							$('#videos_msg').html("<div class='ugc_popdiv'><div class='ugc_popcontent'><div class='ugc_ptext'><h6>Dear "+ fname +"</h6><p>Your video is uploaded successfully. It would be visible on user videos after moderation. Thanks for your patience.</p></div></div></div>").fadeIn('slow');
...
function demougcGetSelectVideoVal(sel)
{
	if (sel.value == '10') {
		$('#video_type').show(); 
	} else {
