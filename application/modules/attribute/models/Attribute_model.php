  ...
  class Attribute_model extends CI_Model
  {
     ...
     {  
 		...
 		if(!empty($frmData['id'])){
			...
			$whereData = array(
             	'product_id' => $frmData['id']
         	);
 
			$this->db->set($UpdateData);
 			$this->db->where($whereData);
 			if($this->db->update('attribute_name')){//echo '===query===='.$this->db->last_query();
...
*/
    ...
    {/*  
 	  ...
 	  //echo '*<pre>';print_r($frmData);exit;
		...
		if(!empty($frmData['group_id'])){
			...
			if($this->db->update('attribute_name')){
	 ...
	 */}
	 ...
	 function deleteMenu($ParentId='', $childId){
 		 $sql = "delete from attribute_name where product_id = '".$ParentId."'";
