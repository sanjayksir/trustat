<?php
 class plant_master_model extends CI_Model
 {
    ...
    { //echo '<pre>';print_r($frmData);exit;
	 ...
	 public function user_registration_mail($plant_code='', $plant_name='', $email)
	 {//echo '***'.$email;exit;
    ...
    { 	//echo '<pre>';print_r($frmData);exit;
	 ...
	 function save_assign_plants_users($plant_array, $users,$is_edit='')
    { 	// echo '<pre>cccccccc';print_r($this->uri->segment(3));exit;
		$plant_arr 		= json_decode($plant_array,true);		 
		$user_id 		= $this->session->userdata('admin_user_id');
