  ...
  */
    ...
    {  
		...
		if(!empty($frmData['menu_id'])){
			...
			if($this->db->insert("menu_master", $insertData)){//echo '===query===='.$this->db->last_query();
	...
	function get_ownership_bkp_new2()
    { 
		$ownership_id = '1';
		$user_id 	= $this->session->userdata('admin_user_id');
    ...
    {  
		...
		if(!empty($id)){
			...
			$this->db->set($UpdateData);
			$this->db->where($whereData);
			if($this->db->update('menu_master_permission')){
		 ...
		 $UpdateData = array("status "=>0); 
 		 if(empty($childId)){ 			
			  $this->db->where(array('id' => $ParentId));
			   $this->db->or_where(array('parent' => $ParentId));
		  }else{
			  $whereData = array('id' => $childId);
			  $this->db->where($whereData);
		 }
 		$this->db->set($UpdateData);
