      ...
      <div class="breadcrumbs ace-save-state" id="breadcrumbs">
        ...
        <ul class="breadcrumb">
          ...
          <li> <i class="ace-icon fa fa-home home-icon"></i> <a href="<?php echo site_url(); ?>">Home</a> </li>
		 				...
		 				<?php if($this->uri->segment(2)=='edit_assign_loyalty_activity_role'){

								  	$constant = "Edit Assign Loyalty to Functionality / Role" ;

                              }elseif($this->uri->segment(2)=='add_assign_loyalty_activity_role'){ // $this->load->view('add_member_right_tpl'); 
							  $constant = "Add Assign Loyalty to Functionality / Role" ;

					 			}else{
								$constant = "View Assign Loyalty to Functionality / Role" ;	
								}
								?>	
                          ...
                          <div class="col-xs-12">
                            ...
                            <div class="row" id="add_edit_div">
                              ...
                              <?php if($this->uri->segment(2)=='edit_assign_loyalty_activity_role'){
								  	...
								  	$this->load->view('edit_assign_loyalty_activity_role_tpl'); 
									
									 }elseif($this->uri->segment(2)=='add_assign_loyalty_activity_role'){
										 
										  $this->load->view('add_assign_loyalty_activity_role_tpl'); 

                              }else{  $this->load->view('view_assign_loyalty_activity_role_tpl'); 
		...
		submitHandler: function(form) {
 		...
 		/*	formData 		= new FormData();
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>plant_master/save_assign_loyalty_activity_role/",
 				data: dataSend,
				success: function (msg) {
					if(parseInt(msg)==1){
						$('#ajax_msg').text("Added Successfully!").css("color","green").show();
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>role_master/list_assigned_functionalities_to_role/<?php echo $this->uri->segment(4); ?>";						
					}
					else if(parseInt(msg)==2){
						$('#ajax_msg').text("Aalready Exists!").css("color","red").show();
  					      window.location.href="<?php echo base_url(); ?>role_master/list_assigned_functionalities_to_role/<?php echo $this->uri->segment(4); ?>";						
					}
					else{
