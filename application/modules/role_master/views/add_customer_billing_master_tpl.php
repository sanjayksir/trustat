        ...
        <ul class="breadcrumb">
          ...
          <li> <i class="ace-icon fa fa-home home-icon"></i> <a href="<?php echo site_url(); ?>">Home</a> </li>
		 				...
		 				<?php 
								...
								if($this->uri->segment(2)=='edit_customer_billing_master'){
 									$constant 		= "Edit " .$type;
									} elseif ($this->uri->segment(2)=='view_customer_billing_master') {
										$constant 		= "View " .$type;
          ...
          <li class="active">Customer Billing Master</li><li class="active"><?php echo $constant;?></li>
                        ...
                        <div class="row">
                          ...
                          <div class="col-xs-12">
                            ...
                            <div class="row" id="add_edit_div">
                              ...
                              <?php if($this->uri->segment(2)=='edit_customer_billing_master' || $this->uri->segment(2)=='view_customer_billing_master'){
									if($this->uri->segment(2)=='edit_customer_billing_master'){
								  	$this->load->view('edit_customer_billing_master_right_tpl'); 
									} else{  $this->load->view('view_customer_billing_master_tpl'); 

					 			}

                              }else{  $this->load->view('add_customer_billing_master_right_tpl'); 
...
<script>
 ...
 $(document).ready(function(){	
	...
	$("form#user_frm").validate({
		rules: {
  			mis_report_name:{
						 ...
						 remote: {
                       	 	...
                       	 	url: "<?php echo base_url().'role_master/';?>checkAutoEmailMISMaster",
                          	type: "post",
 							data: {  r_id: $( "#r_id" ).val() }
                    	 } 
					  },
		...
		messages: {
				mis_report_name: {
					required: "Please Enter Customer Billing Master Name...",
					remote: "This Customer Billing Master Name already exists!"
				}, 
		},
		...
		submitHandler: function(form) {
 		...
 		/*	formData 		= new FormData();
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>role_master/save_customer_billing_master/",
 				data: dataSend,
				success: function (msg) {
					if(parseInt(msg)==1){
						...
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>role_master/list_all_customer_billing_masters/";						
					}
					else if(parseInt(msg)==2){
						$('#ajax_msg').text("Customer Billing Master Already Exists!").css("color","red").show();
  					      window.location.href="<?php echo base_url(); ?>role_master/list_all_customer_billing_masters/";						
					}
					else{
