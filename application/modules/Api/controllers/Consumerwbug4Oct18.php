            ...
            if (strstr($questionType, 'audio')) {
                ...
                $transactionType = 'product_demo_video_response_lps';
				 $transactionTypeName = 'Viewing product video demonstration';
				  } elseif (strstr($questionType, 'demo_audio')) {
                $transactionType = 'product_demo_audio_response_lps';
				 $transactionTypeName = 'Listening product audio demonstration ';
            } else {
                $transactionType = 'product_description_response_lps';
				 $transactionTypeName = 'Genuity Scan and Responding to Product Description';
            }
            //$data['transaction_type'] = $questionType;
            $this->Productmodel->feedbackLoylity($transactionType, $transactionTypeName, $ProductID, $consumer_id, ['verification_date' => date("Y-m-d H:i:s"), 'consumer_id' => $consumer_id, 'consumer_name' => $consumer_name, 'brand_name' => $product_brand_name, 'product_name' => $product_name, 'product_id' => $ProductID, 'product_code' => $data['bar_code']], 'Loyalty');
			
			//$this->Product_model->saveProductLoylty($transactionType, $ProductID, $consumer_id, ['transaction_date' => date("Y-m-d H:i:s"),'consumer_id' => $consumer_id,'product_id' => $ProductID]);
			
			$this->Productmodel->saveConsumerPassbookLoyalty($transactionType, $transactionTypeName, $ProductID, $consumer_id, ['verification_date' => date("Y-m-d H:i:s"), 'consumer_id' => $consumer_id, 'consumer_name' => $consumer_name, 'brand_name' => $product_brand_name, 'product_name' => $product_name, 'product_id' => $ProductID, 'product_code' => $data['bar_code']], 'Loyalty');
