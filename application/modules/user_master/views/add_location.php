		...
		submitHandler: function(form) {
			...
			formData.append('file', $('#file')[0].files[0]);
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>user_master/save_location11/",
 				data: dataSend,
				data: formData,
 				async: false,
 				cache: false,	
				...
				success: function (msg) {
					if(parseInt(msg)==1){
						...
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>plant_master/list_locations/<?php if(($this->uri->segment(2))=="edit_location"){ echo getOwnerCustNameByLocationId($this->uri->segment(3)); }else{ echo $this->uri->segment(3); } ?>";						
					}
					else if(parseInt(msg)==2){
						$('#ajax_msg').text("Locations Aalready Exists!").css("color","red").show();
  					      window.location.href="<?php echo base_url(); ?>plant_master/list_locations/<?php echo $this->uri->segment(3); ?>";						
					}
					else{
