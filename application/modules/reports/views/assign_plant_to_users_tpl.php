 ...
 <div class="main-container ace-save-state" id="main-container"> 
  ...
  <div class="widget-box">
    ...
    <div class="widget-header">
      ...
      <h4 class="widget-title">Assign Plant To User</h4>
...
<?php //$UserData = get_all_users($user_id);
...
//echo '<pre>';print_r($UserData);?>
      <form name="user_frm" id="user_frm" action="#" method="POST" onsubmit="return saved_assigned_data();">
		...
		<div class="form-group row">
			...
			$UserData = get_all_users($user_id);
			//$plant_data = get_all_plants($user_id);
			foreach($UserData as $res){?>
            <option value="<?php echo $res['user_id'];?>" <?php if($this->uri->segment(3)==$res['user_name']){echo 'selected';}?>><?php echo ucfirst($res['user_name']);?></option>
            <?php }?>
            </select>
			...
			<div class="col-sm-6">
			  <label for="form-field-8">Plants</label>
		 ...
		 function get_plants(id){
		 	if(id!=''){
				$.ajax({
				type:'POST',
				url:'<?php echo base_url().'plant_master/getPlantList'?>',
				data:{id:id},
