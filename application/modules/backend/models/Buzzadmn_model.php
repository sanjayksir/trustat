...
class Buzzadmn_model extends CI_Model {
    ...
    function saveIdeaDetail_MAIN($data) {//echo '<pre>';print_r($data);exit;
        $user_id = $this->session->userdata('admin_user_id');
    ...
    function saveIdeaDetail($data) {
       //echo '**<pre>'.print_r($data);exit;
        $checkStoryUpdate = $this->chcekExistsMedia($data['product_id']); 
        $insertData = array(
           // "id" => $data['product_id'],
		   "product_registration_lps" =>  trim($data['product_registration_lps']),
		   ...
		   "feedback_on_product_lps" =>  trim($data['feedback_on_product_lps']),
		   "product_image_response_fbqq" =>  trim($data['product_image_response_fbqq']),
        ...
        if ($checkStoryUpdate == '1') {
            ...
            $checker = 0; //echo '***'.$data['images'];
			 if ($data['pickDesc'] != '') {
                $checker = 1;
                $arr = array("product_description" => trim($data['pickDesc']));
                $update_array = array_merge($update_array, $arr);
            }
            if ($data['ptimages'] != '') {
                $checker = 1;
                $arr = array("product_thumb_images" => trim($data['ptimages']));
                $update_array = array_merge($update_array, $arr);
            }
			if ($data['images'] != '') {
                $checker = 1;
			...
			if ($data['pcpbgimages'] != '') {
                $checker = 1;
                $arr = array("product_code_print_bg_images" => trim($data['pcpbgimages']));
                $update_array = array_merge($update_array, $arr);
            }
            if ($data['videos'] != '') {
                $checker = 1;
                $arr = array("product_video" => trim($data['videos']));
                $update_array = array_merge($update_array, $arr);
            }
            if ($data['audios'] != '') {
                $checker = 1;
                $arr = array("product_audio" => trim($data['audios']));
                $update_array = array_merge($update_array, $arr);
            }
            if ($data['attachments'] != '') {
                $checker = 1;
            ...
            if ($checker == 1) {
                $whereData = array('id' => $data['product_id']);
    ...
    function chcekExistsDetail($product_id) {
        $result = array();
    ...
    function chcekExistsMedia($product_id) {
        if (!empty($product_id)) {
            $this->db->select('id');
            $this->db->from('products');
            $this->db->where(array('id' => $product_id));
    ...
    public function storeSpideyBuzz($array) {  // echo '<pre>kam==';print_r($array);exit;
        ...
        if ($array['video_media']) {
            $buzz_data['video_url'] = trim($array['video_media']);
        }
		 if ($array['videos']) {
            $buzz_data['videos'] = trim($array['videos']);
        }
        ##-------------------- Image Fields----------------------##
        ...
        if ($array['spidypickId'] && $array['spidypickId'] != '' && $array['spidypickId'] > 0) {###---------- EDIT Story CODE
            ...
            $buzz_data['updatedby'] = $this->session->userdata('admin_user_id');
            ...
            $buzz_data['createdby'] = $this->session->userdata('admin_user_id');
