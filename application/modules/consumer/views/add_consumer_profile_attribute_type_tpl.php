        ...
        <ul class="breadcrumb">
          ...
          <li> <i class="ace-icon fa fa-home home-icon"></i> <a href="<?php echo site_url(); ?>">Home</a> </li>
		 				...
		 				<?php 
								...
								if($this->session->userdata('admin_user_id')>1 || $this->uri->segment(2)=='add_plant_controller' || $this->uri->segment(2)=='edit_consumer_profile_attribute_type'){
									$type 			= "Consumer Profile Attribute Types";
									$urlredirect 	= 'list_plant_controllers';
								}
						
								if($this->uri->segment(2)=='edit_consumer_profile_attribute_type'){
 									$constant 		= "Edit " .$type;
									} elseif ($this->uri->segment(2)=='view_consumer_profile_attribute_type') {
										$constant 		= "View " .$type;
                        ...
                        <div class="row">
                          ...
                          <div class="col-xs-12">
                            ...
                            <div class="row" id="add_edit_div">
                              ...
                              <?php if($this->uri->segment(2)=='edit_consumer_profile_attribute_type' || $this->uri->segment(2)=='view_consumer_profile_attribute_type'){
									if($this->uri->segment(2)=='edit_consumer_profile_attribute_type'){
								  	$this->load->view('edit_consumer_profile_attribute_type_right_tpl'); 
									} else{  $this->load->view('view_consumer_profile_attribute_type_tpl'); 

					 			}

                              }else{  $this->load->view('add_consumer_profile_attribute_type_right_tpl'); 
	...
	$("form#user_frm").validate({
		rules: {
  			cpatm_name:{
						 ...
						 remote: {
                       	 	...
                       	 	url: "<?php echo base_url().'consumer/';?>checkProfileAttributeType",
                          	type: "post",
 							data: {  cpatmid: $( "#cpatm_id" ).val() }
		...
		messages: {
				...
				cpatm_name: {
					...
					required: "Please Enter Consumer Profile Attribute Type Name.",
		...
		submitHandler: function(form) {
 		...
 		/*	formData 		= new FormData();
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>consumer/save_consumer_profile_attribute_types/",
 				data: dataSend,
				success: function (msg) {
					if(parseInt(msg)==1){
						...
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>consumer/list_consumer_profile_attribute_types/";						
					}
