...
<?php $this->load->view('../includes/admin_top_navigation');?>
	<div class="main-container ace-save-state" id="main-container">
			...
			if($this->session->userdata('admin_user_id')==1){
				$label = 'Product Returned From Consumer CMS Items';
			}else{
				$label = 'Product Returned From Consumer CMS Items';
			}
			?>
 						...
 						<div class="row">
							<div class="col-xs-12">
 								<div class="widget-box widget-color-blue">
                                                               ...
                                                               <!--  <a href="<?php //echo base_url('consumer/list_consumer_profile_attribute_types') ?>" class="btn btn-xs btn-warning" title="Add User">Add Consumer Profile QUERY <?php // echo $label; ?> </a> |--> <a href="<?php echo base_url('consumer/add_consumer_return_cms_item') ?>" class="btn btn-xs btn-warning" title="Add User">Add <?php echo $label; ?> </a> 
                                                                    </div>
                                                                </div>
            ...
            <form id="form-filter" action="" method="get" class="form-horizontal" >
                ...
                <div class="col-sm-6">
                    <div class="input-group">
                        <input type="text" name="search" id="search" value="<?= $this->input->get('search',null); ?>" class="form-control search-query" placeholder="Product Returned From Consumer CMS Items">
                        ...
                        <table id="missing_people" class="table table-striped table-bordered table-hover">
                            <thead>
                                    <tr>
                                            <th>#</th>
											<th>Product Return CMS Type</th>
                                            <th>Return CMS Type Name Value</th>
                                            <th>Return CMS Type Name Slug</th>                                           
                                            <th>Active Status</th>
											<th>Action</th>
                                    </tr>
                            </thead>
                            <tbody>
                                    ...
                                    if(count($userListing)>0){
										...
										<tr id="show<?php echo $listData['prfcci_id']; ?>">
                                           <td><?php echo $sno;$sno++; ?></td>
										   <td><?php echo $listData['return_cms_type']; ?></td>
                                           <td><?php echo $listData['return_cms_type_name_value']; ?></td>
                                           <td><?php echo $listData['return_cms_type_name_slug']; ?></td>
                                          <td><?php echo $listData['active_status']; ?></td>
                                                 ...
                                                 <td>
                                                    ...
                                                    <div class="hidden-sm hidden-xs action-buttons">
                                                       ...
                                                       <!-- <a href="<?php //echo base_url().'consumer/view_consumer_profile_attribute/'.$listData['prfcci_id'];?>" class="btn btn-xs btn-success" target="_blank" title="View"><i class="fa fa-eye"></i></a>-->
                                                         <?php echo anchor("consumer/edit_consumer_return_cms_items/" . $listData['prfcci_id'], '<i class="ace-icon fa fa-pencil bigger-130"></i>', array('class' => 'btn btn-xs btn-info','title'=>'Edit')); ?>
														 
                                                       <a title="Delete User" href="javascript:void(0);" onclick="return confirmDelete('<?php echo base64_encode($listData['prfcci_id']);?>');" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash-o bigger-120"></i></a>
                                                    </div>
...
function confirmDelete(val='')
{
	if(val!=''){
		var x = confirm("Are you sure you want to delete?");
		 if (x){
		 		window.location.href="<?php echo base_url().'consumer/Del_ConsumerReturnCMSItem/';?>"+val;
 			  //return true;
		 }
		 else{
			return false;
