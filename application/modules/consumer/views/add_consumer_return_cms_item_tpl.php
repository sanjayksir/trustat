        ...
        <ul class="breadcrumb">
          ...
          <li> <i class="ace-icon fa fa-home home-icon"></i> <a href="<?php echo site_url(); ?>">Home</a> </li>
		 				...
		 				<?php 
								...
								if($this->session->userdata('admin_user_id')>1 || $this->uri->segment(2)=='add_plant_controller' || $this->uri->segment(2)=='edit_consumer_return_cms_item'){
									$type 			= "Product Returned From Consumer CMS Item";
									$urlredirect 	= 'list_plant_controllers';
								}
						
								if($this->uri->segment(2)=='edit_consumer_return_cms_item'){
 									$constant 		= "Edit " .$type;
									} elseif ($this->uri->segment(2)=='view_consumer_return_cms_item') {
										$constant 		= "View " .$type;
                        ...
                        <div class="row">
                          ...
                          <div class="col-xs-12">
                            ...
                            <div class="row" id="add_edit_div">
                              ...
                              <?php if($this->uri->segment(2)=='edit_consumer_return_cms_items' || $this->uri->segment(2)=='view_consumer_return_cms_item'){
									if($this->uri->segment(2)=='edit_consumer_return_cms_items'){
								  	$this->load->view('edit_consumer_return_cms_item_right_tpl'); 
									} else{  $this->load->view('view_consumer_return_cms_item_tpl'); 

					 			}

                              }else{  $this->load->view('add_consumer_return_cms_item_right_tpl'); 
...
<script>
 ...
 $(document).ready(function(){	
	...
	$("form#user_frm").validate({
		rules: {
  			return_cms_type_name_value:{
 						 required: true, 
						 remote: {

                       	 	url: "<?php echo base_url().'consumer/';?>checkConsumerReturnCMSItem",
                          	type: "post",
 							data: {  prfcci_id: $( "#prfcci_id" ).val() }
                    	 } 
					  },
			},
		...
		messages: {
				return_cms_type_name_value: {
					required: "Please Enter Product Returned From Consumer CMS Item...",
					remote: "This Product Returned From Consumer CMS Item already exists!"
				}, 
		},
		...
		submitHandler: function(form) {
 		...
 		/*	formData 		= new FormData();
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>consumer/save_consumer_return_cms_item/",
 				data: dataSend,
				success: function (msg) {
					if(parseInt(msg)==1){
						...
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>consumer/list_consumer_return_cms_items/";						
					}
