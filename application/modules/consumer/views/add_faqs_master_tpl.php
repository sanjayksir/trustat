        ...
        <ul class="breadcrumb">
          ...
          <li> <i class="ace-icon fa fa-home home-icon"></i> <a href="<?php echo site_url(); ?>">Home</a> </li>
		 				...
		 				<?php 
								...
								$type 				= "FAQs";
								if($this->session->userdata('admin_user_id')>1 || $this->uri->segment(2)=='add_plant_controller' || $this->uri->segment(2)=='edit_faqs_master'){
									$type 			= "FAQs";
									$urlredirect 	= 'list_plant_controllers';
								}
						
								if($this->uri->segment(2)=='edit_faqs_master'){
 									$constant 		= "Edit " .$type;
									} elseif ($this->uri->segment(2)=='view_faqs_master') {
										$constant 		= "View " .$type;
                        ...
                        <div class="row">
                          ...
                          <div class="col-xs-12">
                            ...
                            <div class="row" id="add_edit_div">
                              ...
                              <?php if($this->uri->segment(2)=='edit_faqs_master' || $this->uri->segment(2)=='view_faqs_master'){
									if($this->uri->segment(2)=='edit_faqs_master'){
								  	$this->load->view('edit_faqs_master_right_tpl'); 
									} else{  $this->load->view('view_faqs_master_right_tpl'); 

					 			}

                              }else{  $this->load->view('add_faqs_master_right_tpl'); 
		...
		rules: {
  			faq_question:{
						 ...
						 remote: {
                       	 	...
                       	 	url: "<?php echo base_url().'consumer/';?>checkFAQs",
                          	type: "post",
		...
		submitHandler: function(form) {
 		...
 		/*	formData 		= new FormData();
 			...
 			$.ajax({
 				...
 				url: "<?php echo base_url(); ?>consumer/save_faqs_master/",
 				data: dataSend,
				success: function (msg) {
					if(parseInt(msg)==1){
						...
						$('#blah').attr('src', '').hide();
						$('#user_frm')[0].reset(); 
						  window.location.href="<?php echo base_url(); ?>consumer/list_faqs_master/";						
					}
					else if(parseInt(msg)==2){
						$('#ajax_msg').text("FAQ Aalready Exists!").css("color","red").show();
  					      window.location.href="<?php echo base_url(); ?>consumer/list_faqs_master/";						
					}
					else{
